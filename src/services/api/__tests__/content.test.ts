...
describe('content api', () => {
  ...
  test('Debería devolver una instrucción con pasos cuando hay data disponible', async () => {
    ...
    expect(result?.updatedAt?.toISOString()).toBe('2026-05-30T02:00:00.000Z');
    expect(result?.steps?.[0].updatedAt).toBeInstanceOf(Date);
    expect(result?.steps?.[0].updatedAt?.toISOString()).toBe('2026-05-30T01:06:00.000Z');
  });
