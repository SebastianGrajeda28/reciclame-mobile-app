...
const BUCKET = "instruction-step-images";

async function uploadStepImage(key: string, file: File): Promise<string> {
...
export function InstructionPreviewRail({
  ...
  const { data: steps = [], isLoading } = useQuery({
    ...
    queryFn: () => getInstructionSteps(session!.access_token, instruction.id),
    enabled: !!session,
    staleTime: 60_000,
    ...
    mutationFn: async () => {
      ...
      const withImages: LocalStep[] = await Promise.all(
        localSteps.map(async (s) => {
          ...
          const url = await uploadStepImage(s.id, s.pendingImageFile);
      ...
      await Promise.all(
        withImages
          ...
          .map((s) => deactivateInstructionStep(token, s.id)),
      );
      ...
      await Promise.all(
        withImages
          ...
          .map(async (s) => {
            const created: InstructionStep = await createInstructionStep(token, {
            ...
            if (s.imageUrl) {
              await updateInstructionStep(token, created.id, { imageUrl: s.imageUrl });
            }
          }),
      );
      ...
      await Promise.all(
        withImages
          ...
          .map((s) => updateInstructionStep(token, s.id, { text: s.text, imageUrl: s.imageUrl })),
      );
      ...
      await updateInstruction(token, instruction.id, { body: encodeStepOrder(finalOrder) });
    },
    onSuccess: async () => {
