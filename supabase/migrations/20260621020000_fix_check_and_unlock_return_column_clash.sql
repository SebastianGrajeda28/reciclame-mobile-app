-- Fix PL/pgSQL variable/column ambiguity: the RETURNS TABLE columns (achievement_id, achievement_name,
-- achievement_slug) clashed with same-named columns in INSERT and SELECT statements inside the function body.
-- Solution: rename return columns to out_achievement_id / out_achievement_name / out_achievement_slug.
-- The TypeScript caller maps by position, not name, so this is safe.
