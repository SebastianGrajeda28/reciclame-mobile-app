-- get_friends_with_profile: expone nivel y calor conservados para colorear Amigos por nivel, no por días.
-- DROP + CREATE: cambia el tipo de retorno (CREATE OR REPLACE no puede).

DROP FUNCTION IF EXISTS public.get_friends_with_profile(uuid);
DROP FUNCTION IF EXISTS app_social.get_friends_with_profile(uuid);
...
REVOKE ALL ON FUNCTION app_social.get_friends_with_profile(uuid) FROM PUBLIC;
GRANT ALL ON FUNCTION app_social.get_friends_with_profile(uuid) TO service_role;
GRANT ALL ON FUNCTION public.get_friends_with_profile(uuid) TO anon;
GRANT ALL ON FUNCTION public.get_friends_with_profile(uuid) TO authenticated;
GRANT ALL ON FUNCTION public.get_friends_with_profile(uuid) TO service_role;
