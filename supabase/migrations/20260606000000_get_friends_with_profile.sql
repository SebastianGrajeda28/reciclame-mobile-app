-- RPC: Obtiene el listado de amigos vinculados con agregados de perfil.
-- Resuelve en una sola consulta: nombre, racha, avatar activo,
-- medallas destacadas y fecha de última actividad.
-- Diseño bilateral: una fila en 'friendships' representa la relación
-- sin importar quién envió la solicitud (user_low / user_high evitan duplicados).
...
-- RPC de auto-prueba: crea dos usuarios, una amistad aceptada, llama a la función
-- y verifica que devuelva exactamente un amigo con los campos requeridos.
-- Limpia todos los datos al finalizar. Espeja el patrón de test_update_featured_medals_flow().
create or replace function public.test_get_friends_with_profile_flow()
returns table(friend_found boolean, has_name boolean, has_streak boolean, message text) as $$
declare
  user_a_id  uuid;
  user_b_id  uuid;
  email_a    text;
